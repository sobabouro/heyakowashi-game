...
//        int[][] vertexSetAry = new int[simpleGeometryVertexGroup.Count - 1][];
...
//        int[] sortedVertxIndexAry = new int[vertexSetAry.Length];

//        辞書でソート最速
//        float[] keys = new float[vertexSetAry.Length];
//        for (int i = 0; i < vertexSetAry.Length; i++) {
//            sortedVertxIndexAry[i] = i;
//        注意点: 1_000_000 は適当なスケーリングファクターで、Y軸の値がX軸の値よりも影響力を持つようにしています。Y軸とX軸の値の範囲に応じて調整が必要です。
//            keys[i] = verticesAry2D[i].y * 1_000_000 + verticesAry2D[i].x; ; // i番頂点のY座標
//        }
...
//        y座標が一番大きい頂点から順に処理
//        foreach (int targetVertexIndex in sortedVertxIndexAry) {
//            for (int i = 0; i < sortedVertxIndexAry.Length - 2; i++) {
//                int[] targetVertexSet = vertexSetAry[sortedVertxIndexAry[i]];
//                Debug.Log("vertexSet: " + string.Join(", ", targetVertexSet.Select(a => a.ToString())));

//                triangleList.Add(new int[] { targetVertexSet[0], targetVertexSet[1], targetVertexSet[2] });
//                頂点をカットして、前後の頂点を辺でつなぎなおす。
//            int[] preVertexSet = vertexSetAry[targetVertexSet[0] - offsetVertexIndex];
//                preVertexSet[2] = targetVertexSet[2];
//                Debug.Log("preVertexSet: " + string.Join(", ", preVertexSet.Select(a => a.ToString())));
//                int[] nextVertexSet = vertexSetAry[targetVertexSet[2] - offsetVertexIndex];
//                nextVertexSet[0] = targetVertexSet[0];
...
//                    水平右側隣接辺を取得
//                    int horizontallyAdjacentEdge = SearchHorizontallyAdjacentEdgeStartVertexIndex(nowVertexIndex, offsetVertexIndex, vertexSetList, verticesAry2D);
//                    ヘルパー頂点を初期化
//                    int helperVertexIndex = InitializeHelperVertexIndex(nowVertexIndex, nextVertexIndex, offsetVertexIndex, verticesAry2D);
//                    頂点を分類
//                    int vertexAssort = DecomposeVertex(preVertexIndex, nowVertexIndex, nextVertexIndex, offsetVertexIndex, verticesAry2D);
//                    頂点座標を取得
//                   Vector2 nowVertex = verticesAry2D[nowVertexIndex - offsetVertexIndex];
//                    頂点情報クラスを登録
//                   AttVertex attVertex = new AttVertex(nowVertex, preVertexIndex, nextVertexIndex, horizontallyAdjacentEdge, helperVertexIndex, vertexAssort);
//                    attVertexAry[nowVertexIndex - offsetVertexIndex] = attVertex;
//                }
//            }

//            Debug.Log(horizontallyAdjacentEdgesAry.Length);
//            Debug.Log("horizontallyAdjacentEdge: " + string.Join(", ", attVertexAry.Select(obj => obj.HorizontalAdjacentEdge.ToString())));
...
//            Debug.Log("VertexAssortAry: " + string.Join(", ", attVertexAry.Select(obj => obj.ToString())));
...
//    public AttVertex(Vector2 position, int preVertexIndex, int nextVertexIndex, int horizontalAdjacentEdge, int helperVertexIndex, int assort) {
