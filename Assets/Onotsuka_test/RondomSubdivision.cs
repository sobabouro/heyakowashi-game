...
public class RondomSubdivision : MonoBehaviour
{
    static Mesh _targetMesh;

    /*  
    UnityEngine.Mesh の 
    public Vector3[] vertices {get; get;} は
    参照渡しで毎回コピーして新配列作ってるから非常に重い．
    はじめに Mesh の情報を static で格納しこれらを使いまわす
    */
    static Vector3[] _targetVertices;
    static Vector3[] _targetNormals;
    static Vector2[] _targetUVs;

    // 平面の方程式はn・r=h(nは法線,rは位置ベクトル,hは定数(=_planeValue))
    ...
    // 切断面に対しての頂点が表裏のどちらであるかの二値判定結果を格納する配列
    static bool[] _isFront;
    // 切断後のMeshでの切断前の頂点の番号を格納する配列
